...
var jfbc = {
    ...
    permissions: {
        ...
        update_scope: function(newScope) {
            ...
            newScope = jfbcJQuery.grep(newScope, function(v, k) { // Create a new scope array with no duplicates
    ...
    opengraph: {
        triggerAction: function(action, href, params) {
            if (params != undefined) {
                params = {
                    params: params
                }; // Wrap it in an extra parameters object so that the query params become an array keyed off params
    ...
    share: {
        ...
        build_with_at_text: function() {
            ...
            if (jfbc.share.friends.tagged !== undefined && !jfbcJQuery.isEmptyObject(jfbc.share.friends.tagged)) {
                ...
                jfbcJQuery.each(jfbc.share.friends.tagged, function(id, values) {
                    friends.append(jfbcJQuery("<a />").addClass("friend").attr({
                        href: values.link,
                        target: "_blank"
                    }).text(values.name).data("fbid", id));
                });
                if (!friends.is(":empty")) {
            ...
            if (jfbc.share.place.tagged !== undefined) {
                component.append(jfbcJQuery("<span />").addClass("place").text(" " + jfbc.share.place.autocomplete.messages.intro_text + " ").append(jfbcJQuery("<a />").attr({
                    href: jfbc.share.place.tagged.link,
                    target: "_blank"
                }).text(jfbc.share.place.tagged.name)));
            }
        ...
        add_composer: function(actionId, capabilities) {
            ...
            var form = jfbcJQuery("<form />").attr({
                id: "jfbc-share-composer"
            }).submit(jfbc.share.form_handler);
            form.append(jfbcJQuery("<input />").attr({
                "id": "jfbc-action-id",
                "type": "hidden"
            }).val(actionId));
            ...
            if (jfbcJQuery.inArray("messages", capabilities) != -1) {
                modal_body.append(jfbcJQuery("<div />").attr("id", "composer-message-group").addClass("control-group")
                    .append(
                        ...
                        jfbcJQuery("<div />").addClass("controls").append(jfbcJQuery("<input />").addClass("input-xxlarge").attr({
                            id: "composer-message",
                            type: "text",
                            maxlength: 1000,
                            autocomplete: "off",
            ...
            if (jfbc.jqcompat)
                ...
                dataDismiss = 'modal';
            modal = jfbcJQuery("<div />").addClass("sourcecoast modal").attr({
                id: "jfbc-share-modal",
                role: "dialog",
                "aria-labelledby": "modal-title"
            });
            modal.append(jfbcJQuery("<div />").addClass("modal-header").append(jfbcJQuery("<button />").addClass("close").attr({
                type: "button",
                "data-dismiss": dataDismiss,
                "aria-hidden": "true"
            }).text("×")).append(jfbcJQuery("<h3 />").attr("id", "modal-title").text(jfbc.share.messages.action_text)));
            modal.append(modal_body);
            modal_body = null;
            modal.append(jfbcJQuery("<div />").addClass("modal-footer").append(jfbcJQuery("<button />").addClass("btn").attr({
                "data-dismiss": dataDismiss,
                "aria-hidden": "true"
            }).text(jfbc.share.messages.close)).append(jfbcJQuery("<button />").addClass("btn btn-primary").attr("type", "submit").text(jfbc.share.messages.action_text)));
            form.append(modal);
        ...
        place: {
            ...
            build_form_fields: function() {
                return jfbcJQuery("<div />").attr("id", "composer-place-group").append(jfbcJQuery("<input />").attr({
                    type: "search",
                    role: "combobox",
                    id: "composer-place-field",
                    autocomplete: "off",
                    placeholder: jfbc.share.place.messages.placeholder,
                    "aria-label": jfbc.share.place.messages.placeholder
                })).hide();
            },
            ...
            autocomplete: {
                ...
                renderItem: function(ul, place) {
                    var li = jfbcJQuery("<li />").addClass("place").attr({
                        "role": "option",
        ...
        friends: {
            ...
            build_form_fields: function() {
                return jfbcJQuery("<div />").addClass("form-inline").attr("id", "composer-friends-group").append(jfbcJQuery("<ul />").addClass("unstyled inline").attr("id", "composer-friends-group-fields").append(jfbcJQuery("<li />").append(
                    jfbcJQuery("<input />").attr({
                        type: "search",
                        role: "combobox",
                        id: "composer-friends-field",
                        autocomplete: "off",
                        placeholder: jfbc.share.friends.messages.placeholder,
                        "aria-label": jfbc.share.friends.messages.placeholder
                    })
                ))).hide();
            },
            build_toggle: function() {
            ...
            autocomplete: {
                ...
                init: function() {
                    ...
                    jfbc.share.friends.autocomplete.search_field.autocomplete({
                        ...
                        select: function(event, ui) {
                            jfbc.share.friends.tagged[ui.item.value] = {
                                name: ui.item.label,
                                link: ui.item.link
                            };
                            jfbc.share.build_with_at_text();
                            jfbcJQuery("#composer-friends-group-fields").prepend(jfbcJQuery("<li />").data("fbid", ui.item.value).addClass("friend").append(jfbcJQuery("<a />").attr({
                                href: ui.item.link,
                                target: "_blank"
                            }).text(ui.item.label)).append(jfbcJQuery("<button />").addClass("btn btn-link").attr({
                                type: "button",
                                title: jfbc.share.friends.autocomplete.messages.remove.replace(/%s/i, ui.item.label)
                            }).text("×").click(jfbc.share.friends.autocomplete.remove_friend)));
                ...
                renderItem: function(ul, friend) {
                    return jfbcJQuery("<li />").addClass("friend").attr({
                        "role": "option",
                        ...
                        jfbcJQuery("<img />").attr({
                            src: (friend.picture === undefined) ? "https:\/\/graph.facebook.com\/" + friend.value + "\/picture" : friend.picture,
                            alt: friend.label,
                            width: 25,
                            height: 25
                        })).append(
    ...
    util: {
        ...
        ajax: function(url, callback) {
            ...
            return jfbcJQuery.ajax({
                url: jfbc.base + 'index.php',
                data: url
            }).done(callback);
        },
        ...
        if (jfbc.login.logged_in && jfbc.login.logout_facebook) {
            jfbcJQuery(document).ready(function() {
                jfbcJQuery(".sclogout-button form").submit(function(e) {
                    e.preventDefault();
                    jfbcJQuery(document).one("jfbc-provider-logout-done", null, {
                        caller: this
                    }, function(e) {
